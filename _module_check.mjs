            ...
            if (ROLL_MIGRATION_ENABLED && ROLL_NUMBER_PATTERN.test(username)) {
                try {
                    ...
                    if (rollSnap.exists()) {
                        ...
                        showError('loginError', 'That roll number has not been verified yet. Log in with your username and link it from your profile.');
                        return;
                    }
                } catch (e) {
        ...
        function closeMigrationModal() {
            window.__migrationGateSkipped = true;
            const modal = document.getElementById('migrationModal');
            if (modal) modal.classList.remove('open');
        }

        async function maybeShowMigrationModal() {
            try {
                const modal = document.getElementById('migrationModal');
                if (!modal || !currentUser) return;
                if (!rollMigrationActive(currentUser)) return;
                if (currentUser.migrationStatus === 'verified') {
                ...
                rollMigrationLog('gate error', e && e.message);
            }
        }
        ...
        async function verifyRollNumber() {
            ...
            if (!rollMigrationActive(currentUser)) { fail('Roll-number linking is not enabled for this session yet.'); return; }
            ...
            rollMigrationLog('claim start', { uid: currentUid, roll: raw });
            ...
            const verdict = evaluateRollClaim(currentUser, rosterEntry);
            rollMigrationLog('identity verdict', { roll: raw, verdict });
            if (verdict.verdict === 'manual-review') {
                fail(verdict.reasons.includes('name-mismatch')
                    ? 'Your account name does not match the roster entry for this roll number. Please contact an administrator for manual verification.'
                    : 'Your roll number could not be automatically verified (branch/name mismatch). Please contact an administrator.');
                await setMigrationState('manual_review', raw, (verdict.reasons || []).join(','));
                renderMigrationStatus();
                return;
            }
        ...
        async function renderAdminUsers() {
            ...
            try {
              ...
              <div style="overflow-x:auto;max-height:500px;overflow-y:auto;">
                <table style="width:100%;border-collapse:collapse;font-size:13px;">
                  <thead style="background:var(--ink);color:#F1ECDD;position:sticky;top:0;">
                    <tr><th style="padding:10px 8px;text-align:left;">Name</th><th style="padding:10px 8px;text-align:left;">Username</th><th style="padding:10px 8px;text-align:left;">Branch</th><th style="padding:10px 8px;text-align:left;">Section</th><th style="padding:10px 8px;text-align:left;">Hostel</th><th style="padding:10px 8px;text-align:left;">Gender</th><th style="padding:10px 8px;text-align:left;">Admin</th><th style="padding:10px 8px;text-align:left;">Requested</th></tr>
                  </thead>
                  <tbody>
            `;
                users.forEach(u => {
